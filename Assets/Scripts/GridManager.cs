...
public class GridManager : MonoSingleton<GridManager>
{
        ...
        for (int x = 0; x < _gridSizeX; x++)
        {
            for (int y = 0; y < _gridSizeY; y++)
            {
                ...
                else
                {
                    ...
                    GridPlan[x, y].CellContentList = new List<ColorInfo.ColorEnum>();
                }
    ...
    public void SpawnHexagon(int index, Vector3 gridPos, Transform parent, Material mat, ColorInfo.ColorEnum color)
    {
     ...
     ColorInfo.ColorEnum GetColorOfHexagonCoordinate(Vector2 coordinate)
    {
        if (IsCoordinateValidAndOpen(coordinate))
        {
            CellData cellData = GridPlan[(int)coordinate.x, (int)coordinate.y];
            if (cellData.CellContentList.Count > 0)
            {
                return cellData.CellContentList[0];
            }
        }
        return ColorInfo.ColorEnum.None;
    }
    

    public void TryMergeHexagons(Vector2 placedHexagonCoordinate)
    {
        ColorInfo.ColorEnum placedHexagonColor = GetColorOfHexagonCoordinate(placedHexagonCoordinate); 
        List<Vector2> hexagonsToMerge = new List<Vector2>();
        HashSet<Vector2> checkedHexagons = new HashSet<Vector2>();
        
        Queue<Vector2> hexagonsToCheck = new Queue<Vector2>();
        hexagonsToCheck.Enqueue(placedHexagonCoordinate);
        checkedHexagons.Add(placedHexagonCoordinate);

        while(hexagonsToCheck.Count > 0)
        {
            Vector2 currentHexagon = hexagonsToCheck.Dequeue();
            hexagonsToMerge.Add(currentHexagon);

            List<Vector2> neighbours = GetNeighboursCoordinates(currentHexagon);
            foreach(Vector2 neighbour in neighbours)
            {
                if (!checkedHexagons.Contains(neighbour))
                {
                    checkedHexagons.Add(neighbour);
                    ColorInfo.ColorEnum neighbourColor = GetColorOfHexagonCoordinate(neighbour); 
                    if(neighbourColor == placedHexagonColor)
                    {
                        hexagonsToCheck.Enqueue(neighbour);
                    }
                }
            }
        }
        
      //  if(hexagonsToMerge.Count > 1)
      //  {
     //       MergeHexagons(hexagonsToMerge,placedHexagonColor);
     //  }
     Debug.Log("Is Using");
    }

    private void MergeHexagons(List<Vector2> hexagonsToMerge,ColorInfo.ColorEnum colorToMerge)
    {
        foreach (Vector2 hexagonCoordinate in hexagonsToMerge)
        {
            CellData cellData = GridPlan[(int)hexagonCoordinate.x, (int)hexagonCoordinate.y];
            for (int i = cellData.CellContentList.Count - 1; i >= 0; i--)
            {
                // Проверяем, нужно ли удалить данный цвет
                if (cellData.CellContentList[i] == colorToMerge)
                {
                    // Destroy(cellData.CellObject); // Если нужно уничтожить объект, раскомментируйте эту строку
                    cellData.CellContentList.RemoveAt(i); // Удаление элемента по индексу
                }
            }
        }
        _stackSpawner.SpawnStacks();
